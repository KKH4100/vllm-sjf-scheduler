...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # SJF needs OPT predictor; other policies use default queue.
        if self.policy == SchedulingPolicy.SJF:
            sjf_model_path = getattr(
                self.scheduler_config, "sjf_model_path",
                "/data_kh/OPT-Predictors/opt-125m-llama3-8b-sharegpt-score-trainbucket10-b32/finetuned"
            )
            self.waiting = create_sjf_queue(sjf_model_path)
            # requests skipped in waiting flow due async deps or constraints.
            self.skipped_waiting = create_sjf_queue(sjf_model_path)
        else:
